<template>
	<div>
		<n-card>
			<n-space>
				<div class="horizontal-layout-group">
					...
					<n-input type="text" placeholder="默认关联主键" autosize style="min-width: 20%;margin-left:10px;" clearable />
					<n-button type="primary" style="margin-left: 10px;" @click="getcolumns">点击对比数据</n-button>
					<n-button type="primary" style="margin-left: 10px;" >点击触发</n-button>
				</div>
			</n-space>
		...
		:single-line="false" :scroll-x="scrollnum"/>
	</div>
</template>
...
let columnindex = 0
let columnkeys=[]//把列名单独存起来，取的时候用columnkeys[columnindex]取，省的数据中缺少字段报错
...
let cellProps = (row: any, rowIndex: any, column: any) => {
	...
	//row中字段存在，判断数据是否相等
	if (row[columnkeys[columnindex]][0]!=row[columnkeys[columnindex]][1]) {
...
export default defineComponent({
	...
	setup() {
		...
		const getcolumns = () => {
			message.loading('正在对比数据，请稍等~', { duration: 2000 });
			axios.get('http://localhost:5000/getcomparedata?templatecode1='+templatecode1+'&templatecode2='+templatecode2).then(
				success => {
					console.log('请求成功！');
					let fuckcolumns = []
